...
{
  /// <summary>
  /// This is a test class for QueryResultParser and is intended
  /// to contain all QueryResultParser Unit Tests
  /// </summary>
  [TestClass]
  public class QueryResultParserTest
  {
    ...
    public void CreateRawStringsEnumeratorTest()
    {
        ...
        FileAccess.Read) )
      {
        using (IEnumerator<string> actual = QueryResultParser.
          CreateRawStringsEnumerator(stream))
    ...
    public void CreateRecordsEnumeratorTest()
    {
      ...
      using (Stream stream = new FileStream(inputFileName, FileMode.Open,
        FileAccess.Read))
      {
        using (IEnumerator<IDictionary<string, string>> actual = QueryResultParser.
          CreateRecordsEnumerator(stream))
        {
          // Reference data
          IDictionary<string, string> record1 = new Dictionary<string, string>()
                             {
                               ...
                               { "developer", null },                     
                               { "qa", null },
                               { "summary", "VPIplayer: the error message comes up during deinstallation: \"-1612...\"" }
                             };
    ...
    /// </summary>
    private static void CheckDictionariesAreEqual(IDictionary<string, string> expected, IDictionary<string, string> actual)
    {
